...
HELP_GENERAL = (
    ...
    "\n"
    "Frosty — это MTProxy для Telegram. Он работает внутри Telegram, поэтому не нужен VPN и не требуется включать/выключать его вручную.\n"
    "\n"
    "<b>Быстрое подключение:</b>\n"
    "После оплаты нажми «🔌 Подключить прокси» — Telegram сам предложит добавить прокси.\n"
    "\n"
    ...
    "\n"
    "1) <b>Сколько устройств можно подключить?</b>\n"
    "Подписка привязана к твоему Telegram-аккаунту, а прокси можно включить на любом твоём устройстве (Android/iOS/Telegram Desktop). Лимит: до 10 устройств на один аккаунт.\n"
    "\n"
    "2) <b>Какие лимиты по трафику?</b>\n"
    "Лимитов по скорости и трафику нет. Ты используешь Telegram как обычно — без дополнительных ограничений.\n"
    "\n"
    "3) <b>Чем отличается от VPN и что обещаем?</b>\n"
    "VPN обычно маршрутизирует весь трафик и может давать просадки из‑за шифрования и лишних маршрутов. Frosty — это прокси только для Telegram, поэтому обычно быстрее и стабильнее, не мешает другим приложениям.\n"
    "\n"
    "<b>Мы обещаем:</b>\n"
    "• 24/7 стабильную работу\n"
...
async def cmd_start(message: Message, session: aiohttp.ClientSession, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "Как Telegram обычно работает ДО подключения:\n"
        "• медиа (фото/видео) может долго грузиться или не открываться\n"
        "• скорость и стабильность часто плавают\n"
        "• приходится постоянно переключать VPN/переподключать\n\n"
        "Как будет ПОСЛЕ подключения платного MTProxy Frosty:\n"
        "• Telegram работает через MTProxy прямо в приложении\n"
...
async def cmd_status(message: Message, session: aiohttp.ClientSession, tg_id: int) -> None:
    ...
    if not data.get("active"):
        await message.answer(
            "Подписка не активна.\n\n"
            "Как Telegram обычно работает ДО подключения:\n"
            "• медиа может долго грузиться или не открываться\n"
            "• скорость и стабильность плавают, иногда приходится переключать VPN\n\n"
            "Как будет ПОСЛЕ оплаты и подключения MTProxy Frosty:\n"
            "• Telegram работает через MTProxy прямо в приложении\n"
            "• медиа загружается стабильнее, скорость ровнее\n"
            "• без VPN и постоянных переключений\n\n"

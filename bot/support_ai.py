...
SYSTEM_PROMPT = f"""Ты — служба поддержки сервиса Frosty: VPN-подписка (VLESS Reality, приложение Happ) за {PRICE_RUB} ₽/мес.
...
- Оплата: мини-приложение Telegram (кнопка «Оформить VPN» в боте), карта или СБП. Для ссылки — get_payment_instructions.
...
- VPN: приложение Happ (Android/iOS). Конфиг — в мини-приложении или кнопка «🛡 Подключить VPN» в боте.
- Telegram, Instagram, TikTok, YouTube — через один VPN-профиль. Не упоминай MTProxy, прокси для Telegram и подписку «2 в 1».
...
async def _call_tool(
    ...
    if name == "get_payment_instructions":
        ...
        if pay_url:
            text = (
                f"Цена {PRICE_RUB} ₽/мес. Оплатите сейчас: откройте в боте кнопку «Оформить VPN» "
...
def _fallback_from_tool_result(raw: str) -> str:
    ...
    if "active" in data:
        ...
        return f"Активной подписки нет. Оплатить — кнопка «Оформить VPN» в меню бота."

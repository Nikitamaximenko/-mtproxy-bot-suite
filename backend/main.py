...
def utcnow() -> datetime:
    ...
    # Always return naive UTC to avoid "can't compare offset-naive and offset-aware datetimes".
...
def _vpn_server_migration_message(name: str = "") -> str:
    ...
    return (
        f"✅ <b>{greeting}Frosty VPN снова работает</b>\n\n"
        "Мы переехали на <b>новый сервер</b> — подключение снова стабильное "
        "(Instagram, YouTube, TikTok и др.).\n\n"
        "⚠️ <b>Старый ключ в Happ больше не работает.</b> "
        "Подписка та же — нужно один раз поставить новый ключ.\n\n"
        "📱 <b>Как обновить:</b>\n"
        "1. Удалите старый профиль Frosty в <b>Happ</b>\n"
        "2. Нажмите <b>«Подключить VPN»</b> ниже\n"
        "3. Скопируйте новый ключ → Happ: «+» → «Вставить из буфера»\n"
        "4. Включите VPN (режим <b>Global</b>, DNS — <b>Remote</b>)\n\n"
        "Вопросы — /support"
    )
